...
def detect_silent_sections(y, threshold_db=-50, min_silence_duration=0.2, tolerance_db=10,tolerance_duration=0.2):
    # Adjust threshold with leniency
    adjusted_threshold_db = threshold_db + abs(tolerance_db)

    # Get intervals of non-silent audio
    intervals = librosa.effects.split(y, top_db=abs(adjusted_threshold_db))

    silent_sections = []
    for start, end in intervals:
        # Calculate silence duration with leniency
        duration = librosa.samples_to_time(end - start)
        if duration >= (min_silence_duration - tolerance_duration):
