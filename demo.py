...
@app.route('/')
def index():
    return render_template('main_demo.html')
...
def process_audio():
    ...
    try:
        ...
        if "help" in question.lower():
            ...
            answer = "Sure, choose any one topic from the following and talk about it for one minute."
...
def analyze_audio(y, sr):
    ...
    return {
        ...
        "stressed_syllables": int(stressed_syllables_count),
        "falling_intonation": bool(falling_intonation),
        "detected_silence_sections": len(detected_silence_sections),
        "number_of_words": int(number_of_words),
        "speech_rate": float(speech_rate),
        "fluency_tempo": float(tempo),
        "zero_crossing_rate": float(zero_crossing_rate),
        "root_mean_square_energy": float(root_mean_square_energy),
        "fluency_improvement_needed": bool(fluency_improvement_needed)
    }
